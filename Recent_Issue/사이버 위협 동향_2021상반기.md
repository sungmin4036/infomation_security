...
CGI 파일은 통신에 필요한 경우 인자 값을 입력받아 처리 할수 있다.
...
- 조치 방안
  ...
  - 16진수 코드 변환 등으로 코드 실행 방지 필요
    - > 일일이 검증이 어려울시 개발환경 별 보안 라이브러리 및 클래스 사용
      ...
      - JAVA : Lucy-xss-servlet-filter. OWASP ESAPI 라이브러리
...
파일을 다운로드시 외부서버에서 파일명, 로컬에 저장될 경로등을 인자값으로 받는다.

![image](https://user-images.githubusercontent.com/62640332/160433041-10bab218-5e64-4d49-95b9-f22376ff0f3d.png)

전달된 외부 서버와 파일명은 아래 그림과 같이 CInternetSesseion:OpenURL() 함수를 통해 HTTP 서비스에 연결하여 외부 URL에 접근하고, CStdioFile:Write()로 전달된 로컬 경로에 파일을 다운로드 한다.



![image](https://user-images.githubusercontent.com/62640332/160433182-2538aa03-cd34-4979-9ccb-90341b859e69.png)


이때 외부 서버 URL 유형성과 다운로드 받을 파일에 검증없이 전달되는 인자값을 그대로 받아 메소드를 실행하기 떄문에 공격자 서버로부터 악성 파일을 사용자 시작 프로그램 경로에 다운로드 도록 하는 공격이 가능하다.

시작 프로그램 경로에 파일이 다운로드 될 경우, 사용자 CPC가 재부팅될때 마다 다운로드 된 파일이 함꼐 실행되기 떄문에 해당 취약점은 파일 다운로드뿐만 아니라 실행까지 이어질수 있다.


![image](https://user-images.githubusercontent.com/62640332/160433407-9e015a13-a5ea-40a3-a84f-240321ee9ae4.png)

- 조치방법
  - ActiveX 내 불필요한 메소드 제거 조치
  - 인자 값으로 외부 서버의 실행 파일이 전달될경우 다운로드가 불가하도록 조치
    - 실행 파일 검증 시 확장자(*.exe, *.dll, *.cmd, *.bat 등) 뿐만 아니라 위회되지 않도록 파일 시그니처(\x4d\x5a)도 검증 필요
    - 꼭 필요한 경우 외부 서버와 다운로드 받을 파일에 대한 검증 로직 추가
      - 다운로드 받아 실행한 파일의 인증서 검증 등 무결성 검증 과정 추가
      - 파일명 추출을 통한 검증이 아닌 URL 끝에서부터 byte 단위로 검증할 경우 공백 등을 통해 공격자가 확장자 등 우회 가능흐모 패치시 주의
    - 시작 프로그램 경로에 파일 다운로드 불가하도록 조치
      - 파일 경로에 'start Meny', 'startUp' 문자열이 존재할 경우 다운로드 불가하도록 조치
      - 추가적으로 아래와 같이 연산자 &&등 을 통해 필터링 우회하여 다운로드 수행할 떄도 있으므로 빠짐없이 연산자 및 경로 탐색 패턴 ../, ..\, \\.. 등 필터링 수행
      - 일부만 검증할 경우 탐지되지 않는 패턴으로 우회학여 공격 하므로 주의

![image](https://user-images.githubusercontent.com/62640332/160434161-9dd3c5e3-f91b-4990-a460-b228c593e49b.png)